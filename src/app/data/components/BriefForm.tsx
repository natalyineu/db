    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input
          ...
          id="platforms-0"
          name="platforms[0]"
          className={`w-full px-3 py-2 border rounded-md ${formErrors['platforms[0]'] ? 'border-red-500' : 'border-gray-300'}`}
          placeholder="https://your-landing-page.com"
          value={formData.platforms[0] || ''}
          onChange={handleChange}
        />
        {formErrors['platforms[0]'] && (
          <p className="mt-1 text-sm text-red-500">{formErrors['platforms[0]']}</p>
        )}
      </div>
      ...
      <div>
        ...
        <input
          ...
          className={`w-full px-3 py-2 border rounded-md ${formErrors.target_audience ? 'border-red-500' : 'border-gray-300'}`}
          placeholder="Example: Women 25-45 interested in fitness"
          value={formData.target_audience}
          onChange={handleChange}
        />
        {formErrors.target_audience && (
          <p className="mt-1 text-sm text-red-500">{formErrors.target_audience}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            id="start_date"
            name="start_date"
            className={`w-full px-3 py-2 border rounded-md ${formErrors.start_date ? 'border-red-500' : 'border-gray-300'}`}
            min={currentDate}
        ...
        <div>
          ...
          <input
            ...
            id="end_date"
            name="end_date"
            className={`w-full px-3 py-2 border rounded-md ${formErrors.end_date ? 'border-red-500' : 'border-gray-300'}`}
      ...
      <div>
        ...
        <select
          id="type"
          name="type"
          className={`w-full px-3 py-2 border rounded-md ${formErrors.type ? 'border-red-500' : 'border-gray-300'}`}
          value={formData.type}
          onChange={handleChange}
        >
      ...
      {formErrors.consent && (
        <p className="mt-1 text-sm text-red-500">{formErrors.consent}</p>
      )}
      ...
      <div className="flex justify-end">
        <button
          ...
          className={`px-4 py-2 text-white bg-indigo-600 rounded-md hover:bg-indigo-700 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
