...
export class CampaignService {
  ...
  static async getCampaignsByUserId(userId: string): Promise<Campaign[]> {
    try {
      ...
      // Transform the data to ensure it matches our Campaign type
      return data.map((item: any) => this.transformCampaignData(item));
    } catch (error) {
      if (DEBUG) {
   ...
   */
  private static transformCampaignData(item: any): Campaign {
    return {
      id: String(item.id || ''),
   ...
   */
  private static transformAssetData(asset: any): CampaignAsset {
    return {
      id: String(asset.id || ''),
